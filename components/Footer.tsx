...
const Footer = () => {
  return (
    <div>
      <div className="flex flex-col justify-center items-center gap-16  pt-20 pb-14 border-t-1 light:border-[#B4B4B4]/50 dark:border-white/20">
          ...
          <div className="flex items-center justify-center flex-wrap lg:grid lg:grid-cols-5 gap-5 text-center">
            {footerLinks.map((item, _) => (
              <Link
                key={_}
                href={item.href}
                className="text-lg text-neutral-900/80 hover:text-neutral-900 hover:underline hover:underline-offset-8 hover:underline-neutral-900 dark:text-neutral-50 dark:hover:text-neutral-200"
              >
                {item.title}
              </Link>
            ))}
